...
import ru.bmstu.rk9.rao.lib.RaoLibStringJoiner.StringFormat;
...
public class TreeBuilder implements Subscriber {
	...
	private boolean parseEntry(Entry entry) {
		final EntryType entryType = EntryType.values()[entry.header
		...
		case SPAWN: {
			...
			case BETTER:
				...
				ActivityCache activity = Simulator.getModelStructureCache().decisionPointsInfo
						.get(dptNumber).activitiesInfo.get(ruleNum);
				...
				final int numberOfRelevantResources = Simulator
						.getModelStructureCache().patternsInfo
						.get(patternNumber).relResTypes.size();
